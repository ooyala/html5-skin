...
DiscoveryPanel = require('../../js/components/discoveryPanel');
...
describe('DiscoveryPanel', function () {
  it('tests displays discovery panel', function () {
    ...
    DOM = TestUtils.renderIntoDocument(<DiscoveryPanel discoveryData={data} />);
    ...
    var discoveryToaster = TestUtils.findRenderedDOMComponentWithClass(DOM, 'discoveryToasterContainerStyle');
    var vidName = TestUtils.scryRenderedDOMComponentsWithTag(discoveryToaster, 'span');
    //loop through all videos, test expected values from mock data
    for (var i=0; i<data.relatedVideos.length; i++) {
      expect(React.findDOMNode(vidImg[i]).getAttribute('src')).toEqual(data.relatedVideos[i].preview_image_url);
      expect(React.findDOMNode(vidName[i]).textContent).toEqual(data.relatedVideos[i].name);
    }
  ...
  it('tests displays discovery panel with countdown clock', function () {
    ...
    //Render discovery panel into DOM
    DOM = TestUtils.renderIntoDocument(<DiscoveryPanel discoveryData={data} playerState="end" />);
