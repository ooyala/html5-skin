...
describe('DiscoveryPanel', function() {
  ...
  it('tests displays discovery panel with countdown clock', function() {
    ...
    // Render discovery panel into DOM
    var wrapper = Enzyme.mount(<DiscoveryPanel {...props} discoveryData={data} playerState="end" />);

    // test clock click
    var discoveryClock = wrapper.find('.oo-discovery-count-down-icon-style');
    expect(discoveryClock).toBeDefined();
    discoveryClock.simulate('click');
  });
});